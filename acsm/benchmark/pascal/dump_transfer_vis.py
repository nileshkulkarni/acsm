...
class CSPTrainer(train_utils.Trainer):
    def define_model(self, ):
        ...
        if opts.ft_pretrain_epochs > 0:
            is_dataparallel_model = self.dataparallel_model('pred', self.opts.ft_pretrain_epochs, network_dir=opts.ft_name)

            if is_dataparallel_model and torch.cuda.device_count() == 1:
                model = torch.nn.DataParallel(model)

            self.load_network(model, 'pred', opts.ft_pretrain_epochs, network_dir=opts.ft_name)
        ...
        if opts.reinitialize_uv_predictor:
            if is_dataparallel_model:
                # pdb.set_trace()
                self.model.module.reinitialize_uv_predictor()
            else:
                self.model.reinitialize_uv_predictor()
    ...
    def init_dataset(self, ):
        ...
        if opts.dataset == 'imgnet_quad':
            self.pascal_dataloader = quad_imgnet_data.quad_imgnet_dataloader(opts)
            self.all_dataloloader = quad_imgnet_data.quad_imgnet_dataloader(opts)
        else:
    ...
    def forward(self, ):
        ...
        self.loss_factors['nmr_uvs'] = predictions_tuple[index].mean(); index+=1
        self.loss_factors['primary_quat_reg'] = predictions_tuple[index].mean(); index+=1
    ...
    def visuals_to_save(self, total_steps, count=None, for_visdom=False):
        ...
        for b in visual_ids:
            ...
            if opts.learn_with_nmr_uvs:
                nmr_uv_render = self.codes_pred['nmr_uv_render'][b].data.cpu().numpy()
                nmr_uv_render = (nmr_uv_render * 255).astype(np.uint8).transpose(0, 2, 3, 1)
                visuals['nmr_render'] = visutil.image_montage(nmr_uv_render, 
                                                              nrow = min(3, 
                                                                     opts.num_hypo_cams//3 + 1))
            ...
            if opts.learn_with_nmr_uvs:
                sdf_uv = self.codes_pred['sdf_nmr_uv'][b].data
                visuals['sdf_overlay_uvmap'] = []
                for cx in range(opts.num_hypo_cams):
                    temp = bird_vis.sample_UV_contour(img_ix, sdf_uv.float()[cx].cpu(), self.sphere_uv_img, mask[b].float().cpu(), real_img=True)
                    temp = visutil.tensor2im([temp])
                    visuals['sdf_overlay_uvmap'].append(temp)
                visuals['sdf_overlay_uvmap'] = np.stack(visuals['sdf_overlay_uvmap'])
                visuals['sdf_overlay_uvmap'] = visutil.image_montage(visuals['sdf_overlay_uvmap'], nrow = min(3, opts.num_hypo_cams//3 + 1))
