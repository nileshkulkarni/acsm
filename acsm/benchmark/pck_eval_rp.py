...
def pck_at_intervals(intervals, error):
    ...
    for interval in intervals:
        accuracy.append(
            float(np.round(np.mean(np.array(error) < interval), 3)))
    return accuracy
...
def benchmark_all_instances(intervals, kpnames, bench_stats, img_size):
    ...
    stats['mean_kp_err'] = [float(t) for t in np.round(
        np.nanmean(mean_kp_error, 0), 4)]
    stats['median_kp_err'] = [float(t) for t in np.round(
        np.nanmedian(mean_kp_error, 0), 4)]
    stats['std_kp_err'] = [float(t)
                           for t in np.round(np.nanstd(mean_kp_error, 0), 4)]
    stats['data'] = {}
    stats['pck'] = {}
...
def benchmark_all_instances_2(intervals, kpnames, bench_stats, img_size, select_kp_ids=None):
    stats = {}
    plot_intervals = [0.025*i for i in range(40)]
    kp_error_nan_mask = bench_stats['kps_err'][:, :, 1]*1
    ...
    if select_kp_ids is not None:
        for group_name in select_kp_ids.keys():
            ...
            stats['eval_params']['{}_acc'.format(group_name)] = pck_at_intervals(
                intervals, samples.tolist())
...
def benchmark_vis_instances(intervals, dist_thresholds, kpnames, bench_stats, img_size):
    ...
    for dx, dist_thresh in enumerate(dist_thresholds):
        ...
        for kpx, kp_name in enumerate(kpnames):
            valid_inds = np.where(bench_stats_kps_error[:, kpx, 2] < dist_thresh)[
                0].tolist()
            ...
            ex = np.array(
                list(set(kps_vis1_ind) - set(kps_vis2_ind))).astype(np.int)
            if len(ex) > 0:
                stats['eval_params'][dx]['correct'][kpx] += np.sum(
...
def run_evaluation(bench_stats, n_iter, results_dir, img_size, kp_names, dist_thresholds, select_kp_ids):
    ...
    pprint.pprint(zip(stats['kp_names'], stats['median_kp_err'],
                      stats['mean_kp_err'], stats['std_kp_err']))
    print('PCK Values')
    ...
    for dx, dthresh in enumerate(dist_thresholds):
        ...
        mean_pck[dx] = {k: np.round(np.mean(np.array(t)), 4)
