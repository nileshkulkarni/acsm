...
class NMR(object):
    ...
    def forward_depth(self, vertices, faces):
        self.faces = chainer.Variable(
            chainer.cuda.to_gpu(faces, self.cuda_device)
        )
        self.vertices = chainer.Variable(
            chainer.cuda.to_gpu(vertices, self.cuda_device)
        )
        self.depth = self.renderer.render_depth(self.vertices, self.faces)
        depth = self.depth.data.get()
        return depth

    def backward_depth(self, grad_depth):
        self.depth.grad = chainer.cuda.to_gpu(grad_depth, self.cuda_device)
        self.depth.backward()
    ...
    def forward_img(self, vertices, faces, textures):
        ...
        self.textures = chainer.Variable(
            chainer.cuda.to_gpu(textures, self.cuda_device)
        )

        self.images = self.renderer.render(
...
class Render(torch.autograd.Function):
    ...
    def __init__(self, renderer, depth_only=False):
    ...
    def forward(self, vertices, faces, textures=None):
        ...
        cuda_device = vertices.get_device()
        vs = vertices.cpu().numpy()
        vs[:, :, 1] *= -1
        fs = faces.cpu().numpy()
        if self.depth_only:
            self.mask_only = False
            depth = self.renderer.forward_depth(vs, fs)
            return convert_as(torch.Tensor(depth), vertices)
    ...
    def backward(self, grad_out):
        ...
        if self.mask_only:
            ...
            grad_verts = self.renderer.backward_depth(g_o)
            grad_verts = convert_as(torch.Tensor(grad_verts), grad_out)
            grad_tex = None
        else:
...
class NeuralRenderer(torch.nn.Module):
    ...
    def __init__(self, img_size=256, device=0):
        ...
        self.renderer.to_gpu(device=device)

        self.proj_fn = geom_utils.orthographic_proj_withz
        self.offset_z = 5.

    def ambient_light_only(self):
        # Make light_intensity_ambientt only ambient.
    ...
    def forward(self, vertices, faces, cams, textures=None, depth_only=False):
        ...
        if textures is not None:
            return Render(self.renderer, depth_only)(verts, faces, textures)
        elif depth_only:
            return Render(self.renderer, depth_only)(verts, faces)
        else:
            return Render(self.renderer, depth_only)(verts, faces)
...
def exec_main():
    ...
    vertices_var = torch.autograd.Variable(
        torch.from_numpy(vertices[None, :, :]).cuda(device=0),
        requires_grad=True
    )
...
def teapot_deform_test():
    ...
    image_ref = torch.autograd.Variable(
        torch.Tensor(image_ref[None, :, :]).cuda(device=0)
    )
    ...
    cams_var = torch.autograd.Variable(
        torch.from_numpy(cams[None, :]).cuda(device=0)
    )
    ...
    optimizer = torch.optim.Adam(
        opt_model.parameters(), lr=1e-3, betas=(0.9, 0.999)
    )
    ...
    for ix in loop:
        ...
        if ix % 20 == 0:
            ...
            scipy.misc.imsave(
                img_save_dir + 'iter_{}.png'.format(ix), im_rendered
            )
        optimizer.step()
        # t1 = time()
