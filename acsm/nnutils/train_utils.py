...
flags.DEFINE_integer(
    ...
    'If >0, we will pretain from an existing saved model.'
)
flags.DEFINE_float('learning_rate', 0.0001, 'learning rate')
flags.DEFINE_float('learning_rate_mx', 1.0, 'learning rate multiplier')
flags.DEFINE_float('beta1', 0.9, 'Momentum term of adam')
flags.DEFINE_bool(
    'use_sgd', False,
    'if true uses sgd instead of adam, beta1 is used as mmomentum'
)

flags.DEFINE_integer('batch_size', 16, 'Size of minibatches')
flags.DEFINE_integer(
    'num_iter', 0, 'Number of training iterations. 0 -> Use epoch_iter'
)
...
flags.DEFINE_integer(
    'save_latest_freq', 1000, 'save latest model every x iterations'
)
...
flags.DEFINE_integer('save_visual_count', 2, 'visuals save count')
...
# flags.DEFINE_boolean('debug_tb', False, 'use tensorboard to debug?')
...
flags.DEFINE_integer(
    'lr_step_epoch_freq', -1, 'Reduce LR by factor of 10 every k ephochs'
)
...
class Trainer():
    ...
    def load_network(
        ...
        if network_dir is None:
            ...
            network_dir = os.path.join(self.opts.checkpoint_dir, network_dir)
    ...
    def save(self, epoch_prefix):
        '''Saves the model.'''
        self.save_network(
            self.model, 'pred', epoch_prefix, gpu_id=self.opts.gpu_id
        )
        return
    ...
    def init_training(self):
        ...
        if opts.use_sgd:
            self.optimizer = torch.optim.SGD(
                ...
                lr=opts.learning_rate * opts.learning_rate_mx,
                momentum=opts.beta1
            )
        else:
            ...
            self.scheduler = torch.optim.lr_scheduler.StepLR(
                self.optimizer, step_size=opts.lr_step_epoch_freq, gamma=0.8
            )
    ...
    def register_scalars(self, sc_dict, beta=0.99):
        ...
        for k in sc_dict:
            if k not in self.sc_dict:
                ...
                self.sc_dict[k] = beta * self.sc_dict[k] + (1 -
                                                            beta) * sc_dict[k]
    ...
    def train(self):
        ...
        # if opts.debug_tb:
        #     self.tb_visualizer = tensorboard.TBVisualizer(opts)
        for epoch in range(opts.num_pretrain_epochs, opts.num_epochs):
            ...
            for i, batch in enumerate(self.dataloader):
                ...
                if not self.invalid_batch:
                    ...
                    self.forward()
                    self.smoothed_total_loss = self.smoothed_total_loss * \
                        0.99 + 0.01*self.total_loss.item()
                    self.total_loss.backward()
                    self.grad_norm = self.compute_grad_norm(
                        self.model.parameters()
                    )
                ...
                if opts.save_visuals and (
                    total_steps % opts.save_visual_freq == 0
                ):
                    visualizer.save_current_results(
                        total_steps, self.visuals_to_save(total_steps)
                    )

                if opts.print_scalars and (total_steps % opts.print_freq == 0):
                    scalars = self.get_current_scalars()
                    time_diff = time.time() - start_time
                    visualizer.print_current_scalars(
                        time_diff, epoch, epoch_iter, scalars
                    )
                    if opts.plot_scalars:
                        visualizer.plot_current_scalars(
                            epoch,
                            float(epoch_iter) / dataset_size, opts, scalars
                        )

                    # if opts.debug_tb:
                    #     # self.tb_visualizer.log_grads(self.model, total_steps)
                    #     for key in self.codes_pred['logs']:
                    #         self.tb_visualizer.log_histogram(self.codes_pred['logs'], key, total_steps)
