...
class HTML:
    ...
    def add_images(self, tuples):
        ...
        with tag('html'):
            with tag('head'):
                with tag('script', src="https://cdn.jsdelivr.net/npm/vanilla-lazyload@12.1.0/dist/lazyload.min.js"):
                    temp = 1
                            ...
                            for key in keys:
                                with tag('td'):
                                    ...
                                    if key == 'aa_id' or key == 'a_step' or 'ind' in key:
                                        ...
                                        with tag('img', 'data-src=\"{}\"'.format(img_rel_path), width="320px",):
                                            ctr += 1    
                                        # text?('<img width="320px" data-src={} />'.format(img_rel_path))
                                        # pdb.set_trace()
                                        # with tag('img', width="320px", src=img_rel_path):
                                        #     ctr += 1
                with tag('script'):
                    text("var LazyLoadInstance = new LazyLoad();")
